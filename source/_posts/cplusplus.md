...
移动语义的实现需要使编译器知道什么时候需要进行复制，什么时候不需要、只需要调整记录；

常规复制构造函数使用`const`左值引用作为参数，进行深度复制，如语句`#1`；

移动构造函数使用右值引用作为参数（不使用`const`修饰，因为可能需要修改实参），调整记录，转移管理权；
...
```c++
decltype (x) y;   //使y的类型与x相同，x是一个表达式
```

在定义模板是特别有用，因为模板只有实例化是才能确定类型：

```c++
template <typename T,typename U>
void df(T t,,U u){
	decltype( T*U ) tu;  // tu的类型为 T*U 的类型
}
```

3. <span id='jump1'>返回类型后置</span>

C++11新增的函数声明语法：在函数名和参数列表后面指定返回类型；
